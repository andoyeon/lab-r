...
str(wbcd)
# 첫번째 컬럼은 진단 결과이기 때문에 정규화 대상이 아님
# lapply(데이터프레임, 함수이름)
...
# k-NN 알고리즘이 어느 정도의 정확도를 갖는지 테스트하기 위한 데이터

# 학습 데이터/테스트 데이터의 진단 정보를 가지고 있는 벡터(정답)
wbcd_train_lable <- wbcd[1:469, 1]  # 학습 데이터 정답지
table(wbcd_train_lable)

wbcd_test_lable <- wbcd[470:569, 1] # 테스트 데이터 정답지
table(wbcd_test_lable)
...
train_lable <- wbcd[1:469, 1]
test_lable <- wbcd[470:569, 1]

predict <- knn(train = train_data,
               test = test_data,
               cl = train_lable,
               k = 11)
CrossTable(x = test_lable,
           y = predict,
           prop.chisq = F)
