...
# 3
model <- mpg %>% filter(manufacturer %in% c("chevrolet", "ford", "honda"))
mean(model$hwy)


# 데이터 프레임에서 변수를 선택하는 함수 - dplyt::select()
...
# 2
mpg2 <- mpg2 %>% mutate(avg = sum / 2)

# 3
mpg2 %>% arrange(desc(avg)) %>% head(n = 3)
mpg2
...
# 2) compact만 선택한 데이터 프레임에서 제조사별로 그룹 -> 갯수 -> 정렬
mpg %>% filter(class == "compact") %>% 
