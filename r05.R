...
# mean: 평균
# 3rd Quartile: 3사분위(오름차순 정렬했을 때 75%)
# max: 최댓값
summary(exam)


# gglpot2 패키지에는 mpg 데이터 프레임이 있습니다.
# as.data.frame(변수): 
...
# dplyr::rename() 함수는 
# 변수 이름들을 바꾼새로운 데이터 프레임을 생성해서 반환(리턴)
...
# 20 이상, 30 미만이면, 2등급
# 20 미만이면, 3등급
mpg$grade <- ifelse(mpg$avg_mpg >= 30, 1, ifelse(mpg$avg_mpg >= 20, 2, 3))
head(mpg)
tail(mpg)
