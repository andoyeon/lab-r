...
max(mydata$price)
min(mydata$price)
# 범위(range): 최대솟값과 최댓값을 출력
range(mydata$price)

# IQR(Inter-Quartile Range): 3rd Qu. - 1st Qu.
IQR(mydata$price)
# Qurtile: 1/4, 1/2, 3/4 위치의 값들
# 0.25 (25%) 위치: 1사분위(1st qurtile)
# 0.5 (50%) 위치: 2사분위(2nd qurtile), 중앙값(median)
# 0.75 (75%) 위치: 3사분위(3rd qurtile)
...
library(ggplot2)
ggplot(data = mydata, mapping = aes(y = price)) +
  geom_boxplot()

ggplot(data = mydata, mapping = aes(y = mileage)) +
  geom_boxplot() +
  ylab("주행 거리") +
  ggtitle("주행거리 상자 그림")
...
# 그 구간에 해당하는 데이터들의 갯수(빈도수) 막대로 그린 그래프
