...
# 2) 세로로 합치기 - 행을 이어 붙이기 
#    어떤 데이터가 행의 갯수가 너무 많아서 여러 개의 파일로 쪼개져
...
# 연습문제
fl_price <- data.frame(fl = c("c", "d", "e", "p", "r"),
...
# 1
mpg1 <- mpg
mpg1 <- left_join(mpg1, fl_price, by = "fl")
head(mpg1)

# 2
mpg1 %>% select(model, fl, price.x) %>% 
  head()


# 연습문제
medwest <- as.data.frame(ggplot2::midwest)
str(medwest)
# 1
ex <- medwest
ex <- ex %>% 
...
# 3
ex %>% mutate(grade = ifelse(popyoung >= 40, "large",
                             ifelse(popyoung >= 30, "middle", "small"))) %>% 
  group_by(grade) %>% 
  summarise(count = n())

# 4
ex %>% mutate(asian_ratio = popasian / poptotal * 100) %>% 
