...
# income 변수의 데이터 타입은 숫자 -> 양적 변수(quantitative variable)
# income 변수의 요약 정보
...
# 연령대별 성별 평균 급여 그래프
income_by_gender_age <- welfare %>% 
  filter(!is.na(income)) %>% 
  group_by(age, gender) %>% 
  summarise(mean = mean(income))
head(income_by_gender_age)
tail(income_by_gender_age)

ggplot(data = income_by_gender_age, 
...
ggplot(data = income_by_age_range, mapping = aes(x = age_range, y = mean)) +
  geom_col()
...
ggplot(data = income_by_age_range_gender, mapping = aes(x = age_range, y = mean, fill = gender)) +
  geom_col(position = "dodge") +
  theme_wsj() +
...
ggplot(data = job_top10, mapping = aes(x = reorder(job, -count), y = count)) +
  geom_col() +
  coord_flip()
...
male_job_top10
ggplot(data = male_job_top10, mapping = aes(x = reorder(job, count), y = count)) +
  geom_col() +
  coord_flip() +
  xlab("직종") + ylab("인구수")
...
ggplot(data = female_job_top10, mapping = aes(x = reorder(job, count), y = count)) +
  geom_col() +
  coord_flip() +
  xlab("직종") + ylab("인구수")
